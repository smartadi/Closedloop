...
\todo{4D/E PENDING (2026-09-07) --- AL deciding from the plots. The quartile rejection slopes came back NULL on the 13-session cohort (disturbance-rejection RMSE vs pre-stim state, [+1,+3]s: motion OL-vs-CL signed-rank $p = 0.055$, delta $p = 0.79$; only initial deviation flattens, $p = 0.013$). So the OLD ``rejection degrades with state under OL, robust under CL'' claim below is NOT supported --- rewrite as state-INVARIANT-with-constant-offset, or cut D/E. Paragraph retained verbatim only as a placeholder; do not treat as final.}
...
\todo{ASSEMBLY (2026-09-07 reframe). 13-session cohort (m1--m13; new-rig m14/m15 excluded); motion/spectrum panels the 9 with motion (513 trials). Panels regenerate via \texttt{controller-analysis/build\_figure4.m}: B error-composition = \texttt{cl\_rmse\_factor\_windows.m} (decomp bars + exemplar spectra) + \texttt{cl\_mse\_exemplars.m} + \texttt{cl\_delta\_burst\_explore.m}; C rejection anchor = \texttt{ctrl\_disturbance\_rejection.m} (RMSE-to-ref, 13/13, 1.48$\times$). Panel A trial-average decoupling is single-session (bootstrap CI touches 0) --- fill from the cross-session sweep or move to supplement. D/E pending (see the 4D/E todo above). Current image is a labeled PLACEHOLDER; assemble \texttt{Figure4.pdf} at paperStyle once A and D/E are settled. Supplementary: locked summary panels (\texttt{f4p1\_error\_decomp}, \texttt{factor\_slope\_*}); contra$\rightarrow$ipsi predictor CV-$R^2$ + distributed co-suppression (\texttt{stim\_network\_coupling.m}) support treating $G$ as a shared-network disturbance.}
