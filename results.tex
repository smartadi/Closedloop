...
Sustained open-loop stimulation over the 3\,s trial window slightly reduced across-trial variance as the stimulation period progressed. We quantified this as the slope of across-trial variance during the stimulation window ($-0.57$), compared to near-zero slopes in the pre- and post-stimulation windows ($-0.04$ and $0.13$; $n = 13$ sessions, Fig.~\ref{fig:figure2}D). This open-loop reduction was not significant on its own, indicating that the optogenetic input did not destabilize the system; a significant, window-specific reduction in across-trial variance emerged only under closed-loop feedback, quantified by the open-loop/closed-loop variance ratio below (Fig.~\ref{fig:figure3}I).
\todo{(2026-09-08) Step response DROPPED from Fig 2 (user: ``doesn't add anything''): removed old panel D (3-session step-response LTI fit) + its paragraph; the impulse/step LTI claim in the section-opening sentence is now impulse only; the 15-session 2I OL-average LTI fit is also dropped. Old panel E (across-trial variance) relabeled to D. THREE LOOSE ENDS: (1) the integral term was previously motivated by session-to-session offset bias in the step response --- that motivation is now gone from Results; add a one-line motivation if wanted. (2) Methods still has a ``Step response protocol'' subsubsection; its variance-across-trials part underpins panel D and must stay, but the step-response/steady-state-gain sentences are now unused --- trim them. (3) This paragraph still reads $n = 13$ with slopes $-0.57/-0.04/0.13$, but panel D and its caption now say 15 sessions --- recompute the variance slopes at 15 sessions or revert the caption to 13.}
...
\todo{Spectral-attribution results to run/write: on motion-clean trials only (z-scored motion energy $\leq 1.5$), test whether elevated pre-stimulus \emph{relative 2--4\,Hz power} (2--4\,Hz band power divided by 0.4--10\,Hz total, over the $-2$\,s to trial-end window; canonical CL delta state, \texttt{utils/cl\_reldelta.m}) accounts for the top-quartile high-error closed-loop trials, and report the fraction of high-error closed-loop trials coinciding with elevated relative 2--4\,Hz power. Use the relative (power-independent) ratio, not absolute power (the 2026-07-01 signal-power control). Note the quartile analysis already shows closed-loop RMSE climbing with this state (Q1$\rightarrow$Q4 $+0.38$, $p=2.3\times10^{-8}$; survives a pre-stim-only window, $+0.17$, $p=0.009$). Representative low-/high-error single-trial spectra go to the supplementary figure (Fig.~S\ref{fig:lowfreq_examples}; example trials TBD by AL). Methods written in Sec.~\ref{sec:disturbance}.}
...
We first asked, at the level of the whole trial, whether this pre-stimulus state predicts the outcome. Using the level of the contralateral-derived prediction in the second before stimulation as the state, the magnitude of the actual ipsilateral response over the steady-state window (1--3\,s) scaled steeply with state under open loop but was nearly flat under closed loop (Fig.~\ref{fig:figure4}A). \todo{Panel A cross-session number (2026-09-08): if the state is the pre-stim initial deviation, the quartile analysis gives OL RMSE climbing $+0.48$ (Q1$\rightarrow$Q4) vs CL $+0.32$, per-session $|$gap$|$ $0.67\rightarrow0.21$, signed-rank $p=0.0012$, $n=15$ (ctrl\_distrej\_quartiles.m). This resolves the earlier single-session/bootstrap-CI-touches-0 gap. If panel A must instead use the contra-derived prediction \emph{level} as the state (as the sentence says), recompute the quartile split on that state --- currently only the init-dev state has been run across all 15 sessions. Decide which state variable panel A shows, then quote its numbers here as quartiles, not a slope difference.} Feedback thus decouples the trial outcome from the ongoing brain state that otherwise sets it.
