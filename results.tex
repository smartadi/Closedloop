...
\begin{figure}[h]
    ...
    \small{13 controller sessions (2 mice); panels requiring motion or spectra use the 9 sessions with motion capture (513 closed-loop trials). Open loop red, closed loop blue; steady-state window 1--3\,s after laser onset. Single-trial insets from a representative session (AL\_0033, 2025-02-26).\\
    ...
    \textbf{C:} Residual decomposition and rejection. The controlled response $A$ splits into the Global disturbance $G$ (predicted from the contralateral hemisphere with no controller input) and the Local component $L = A - G$ (laser and controller effect), shown on one representative trial. Per-trial RMSE to the reference over 1--3\,s is lower in closed loop in all 13 sessions (median ratio $1.48\times$, Wilcoxon signed-rank $p = 2.4\times10^{-4}$).\\
