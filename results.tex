...
% ------------------------------------------------------------
Trial-averaged cortical responses to optogenetic stimulation followed a linear input--output map, and a low-dimensional linear time-invariant (LTI) system closely approximated the impulse response. We delivered optogenetic impulse inputs at discrete amplitudes spanning 0--1.8\,mW, randomly interleaved across trials within each session. Fig.~\ref{fig:figure2}A shows the trial-averaged impulse responses for one session, from which we estimated a settling time of $\sim$200\,ms.
...
Sustained open-loop stimulation over the 3\,s trial window slightly reduced across-trial variance as the stimulation period progressed. We quantified this as the slope of across-trial variance during the stimulation window ($-0.53$), compared to shallower slopes in the pre- and post-stimulation windows ($-0.23$ and $+0.18$; $n = 15$ sessions, Fig.~\ref{fig:figure2}D). This small during-stimulation reduction (signed-rank $p = 0.048$) shows that the optogenetic input reduces rather than amplifies across-trial variability and does not destabilize the system; the substantially larger, window-specific reduction is the closed-loop effect, quantified by the open-loop/closed-loop variance ratio below (Fig.~\ref{fig:figure3}I).
% NOTE (2026-09-08): Step response dropped from Fig 2 (user: "doesn't add anything"). Removed old
% panel D (3-session step LTI fit) + paragraph; section-opening LTI claim is now impulse-only; the
% 15-session 2I OL-average LTI fit is also dropped; old panel E (variance) relabeled to D. Loose ends
% resolved: integral-term motivation lives in Methods (eq:pi); Methods "Step response protocol"
% retitled/trimmed to the sustained-stimulation variance protocol; variance slopes recomputed at 15
% sessions (pre -0.23 / during -0.53, signed-rank p=0.048 / post +0.18).

Batch-mean $\Delta F/F$ variance during spontaneous activity decreased with trial count and stabilized beyond approximately 50 trials per session (Fig.~\ref{fig:S1}), consistent with the trial-averaging assumption (see Materials and Methods). Because variance convergence alone is a central-limit effect that does not exclude a slowly drifting mean, we additionally verified that the per-trial spontaneous mean was stationary across acquisition order: in 14 of the 15 sessions the per-trial mean showed no significant trend with trial index (Fig.~\ref{fig:S2}; linear regression and Spearman rank correlation both n.s.), slope signs were balanced across sessions (9 positive, 6 negative; sign test $p = 0.61$), and the median session-long drift was only $0.17$ times the trial-to-trial standard deviation. The single exception was one of the two newly added dual-opsin sessions (AL\_0051), which showed a significant upward drift (linear regression $p < 0.001$; drift $1.63$ SD); excluding it leaves every downstream result unchanged. The spontaneous mean is therefore stationary in the remaining sessions, supporting the zero-mean disturbance assumption that underlies the trial-averaging argument. Convergence rates differed across sessions recorded from the same region, consistent with session-to-session differences in brain state.

% ------------------------------------------------------------
\subsection*{Closed-loop control improves reference-tracking accuracy}
% ------------------------------------------------------------
Across 15 sessions from 4 mice, with confirmed opsin expression, closed-loop feedback consistently improved tracking of a reference target activity relative to open-loop stimulation (Fig.~\ref{fig:figure3}). Under open-loop conditions, the feedforward term produced responses that systematically deviated from the reference, reflecting the inability of a fixed input to compensate for ongoing spontaneous fluctuations or for inaccuracies in the estimated open-loop controller. Under closed-loop control, the proportional term corrected instantaneous deviations, while the integral term accumulated and eliminated persistent offset, yielding substantially improved alignment with the target reference at the single-trial level (Fig.~\ref{fig:figure3}A).


\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{images/Figure3_extra.pdf}
    \caption{\textbf{Closed-loop versus open-loop cortical activity control} (4 mice, 15 sessions).\\
    \small{
    ...
    \textbf{F:} Cross-trial variance during the stimulation window, averaged across all 15 sessions, for open-loop and closed-loop conditions.\\
    \textbf{G:} Per-trial RMSE distributions pooled across all 15 sessions, shown as violin plots.\\
...
% ------------------------------------------------------------
To identify what drives residual trial-to-trial tracking error, we first asked whether movement explained high-error trials and whether closed-loop feedback rejected this disturbance. Within each session we regressed per-trial tracking RMSE on concurrent motion energy, separately for open- and closed-loop trials, with RMSE z-scored within each session so that slopes were comparable across sessions ($n = 11$ sessions with simultaneous motion capture). Under open-loop control, tracking error increased with motion (median slope $+0.69$ standard-deviation units of RMSE per standard-deviation of motion; Wilcoxon signed-rank $p = 0.001$), whereas under closed-loop control this dependence was abolished (median slope $-0.02$; $p = 0.90$). The open- versus closed-loop slope difference was significant (signed-rank $p = 0.002$; the open-loop slope exceeded the closed-loop slope in 10 of 11 sessions), and within the highest motion quartile open-loop error was markedly elevated while closed-loop error remained low (rank-sum $p = 7 \times 10^{-15}$; Fig.~\ref{fig:figure_motion}). Movement is therefore a principal disturbance under open-loop stimulation, and feedback control specifically rejects it, decoupling tracking accuracy from motor state.

A different disturbance drove the error that feedback could not remove. On motion-clean trials (z-scored motion energy $\leq 1.5$; 575 closed-loop trials across the 11 motion sessions), we characterized the pre-stimulus state by the relative 2--4\,Hz power of the readout (2--4\,Hz band power divided by 0.4--10\,Hz total power, over the $-2$\,s to trial-end window; a power-independent measure, Sec.~\ref{sec:disturbance}). Per-trial closed-loop tracking error rose with this state (Spearman $\rho = 0.18$, $p = 1.4\times10^{-5}$), and $58\%$ of the highest-error-quartile trials carried elevated (above-median) relative 2--4\,Hz power, versus $47\%$ of the remaining trials. Binned into quartiles of the state, closed-loop RMSE climbed monotonically (lowest to highest quartile, $+0.38$ within-session standard-deviation units, $p = 2.3\times10^{-8}$; the effect persisted under a strictly pre-stimulus window, $+0.17$, $p = 0.009$), whereas open-loop error was flat in it. Unlike movement, this low-frequency cortical fluctuation is therefore not rejected by feedback---it is the component of the ongoing brain state that sets the irreducible closed-loop error. \todo{Supplementary Fig.~S\ref{fig:lowfreq_examples}: representative low-/high-error single-trial spectra --- example trials TBD by AL.}
...
\begin{figure}[h]
    ...
    closed-loop (blue) trials pooled across 11 sessions with motion capture.
...
We first asked, at the level of the whole trial, whether this pre-stimulus state predicts the outcome. Using the level of the contralateral-derived prediction in the second before stimulation as the state, the magnitude of the actual ipsilateral response over the steady-state window (1--3\,s) scaled steeply with state under open loop but was nearly flat under closed loop (Fig.~\ref{fig:figure4}A). \todo{Panel A (2026-09-08): A uses the contra-derived prediction \emph{level} as the state and is trial-averaged; it is currently single-session (the cross-session contra-predictor sweep is SVD-limited --- most controller sessions are not decomposed). The pre-stim \emph{initial-deviation} decoupling now lives in panel D as quartiles (OL climb $+0.48$ vs CL $+0.32$, per-session gap $0.67\rightarrow0.21$, signed-rank $p=0.0012$, $n=15$; ctrl\_distrej\_quartiles.m). Decide: keep A as the single-session contra-level example, run the contra-level quartile split across the SVD-available sessions, or drop A in favour of D. Either way quote quartiles, not a slope difference.} Feedback thus decouples the trial outcome from the ongoing brain state that otherwise sets it.

To identify which state factors drive the residual closed-loop error, we decomposed per-trial tracking error (RMSE) into the unique contribution of three pre-stimulus factors---initial deviation, motion energy, and relative 2--4\,Hz power---resolved by error window (early transient, 0--1\,s; settled, 1--3\,s; 613 closed-loop trials, 11 sessions with motion capture). The composition turned over between windows. The initial deviation dominated the transient but its unique contribution collapsed once activity settled (unique $R^2 = 0.38 \rightarrow 0.03$), whereas relative 2--4\,Hz power carried an increasing share and accounted for most of the explained error in the settled window (unique $R^2 = 0.09 \rightarrow 0.12$). Motion contributed negligibly in either window (unique $R^2 < 0.01$), consistent with feedback having already rejected the motor disturbance (Fig.~\ref{fig:figure_motion}). The error that feedback cannot remove is therefore not a transient leftover but a distinct quantity tied to ongoing low-frequency activity (Fig.~\ref{fig:figure4}B).

We then resolved the mechanism at the single-trial level. Applying the predictor on every stimulation trial defines the \emph{Global} component $G$---the activity the ipsilateral region would have shown with no controller input, that is, the disturbance---and the \emph{Local} component $L = A - G$, the effect of the laser and controller on top of it (Fig.~\ref{fig:figure4}C). Across trials the actual response was well reconstructed as the Global disturbance plus the average local response (cross-validated $R^2 = 0.85$ open loop, $0.78$ closed loop), confirming that $A = G + L$ captures the single-trial response. We then quantified how well each mode holds the readout at the reference against this disturbance, using per-trial RMSE to the reference ($r = -5\%\,\Delta F/F$) over the steady-state window (1--3\,s, after the inhibitory transient). Closed-loop control rejected the disturbance substantially better than open loop, holding the readout closer to the reference in every one of the 15 sessions (open loop $2.61 \pm 0.18$ versus closed loop $1.89 \pm 0.21$\,\%\,$\Delta F/F$, mean\,$\pm$\,SEM across sessions; median across-session ratio $1.39\times$; Wilcoxon signed-rank $p = 6.1\times10^{-5}$; Fig.~\ref{fig:figure4}C). The two dual-opsin sessions added last (AL\_0048, AL\_0051) show the weakest per-session rejection, consistent with their different preparation, but do not reverse the effect.

\todo{4D/E UPDATED (2026-09-08, 15-session cohort, QUARTILE CLASSES not linear fits --- ctrl\_distrej\_quartiles.m). Disturbance-rejection RMSE (dFk$\rightarrow$ref, [+1,+3]s), z-scored within session, binned by pre-stim state quartile, OL vs CL. RESULT is now a clean two-mechanism split: (i) INITIAL DEVIATION decouples --- OL RMSE climbs Q1$\rightarrow$Q4 ($+0.48$, $p=2.2\times10^{-6}$), CL climbs far less ($+0.32$); per-session $|$Q4$-$Q1$|$ gap $0.67\rightarrow0.21$, smaller under CL in 13/15, signed-rank $p=0.0012$. (ii) 2--4\,Hz DELTA is IRREDUCIBLE --- OL is flat in delta ($+0.04$, $p=0.27$) but CL RMSE CLIMBS with delta ($+0.29$, $p=3.1\times10^{-5}$); gap $0.29\rightarrow0.43$ (larger under CL), $p=0.93$. (iii) MOTION is marginal ($n=11$: OL climb $+0.20$, $p=0.088$; gap $0.70\rightarrow0.30$, $p=0.10$) --- send to supplement. RECOMMENDED framing: REWRITE, do not cut --- panel D = init-dev quartiles (feedback removes dependence on the error it senses), panel E = delta quartiles (the residual it cannot). Body paragraph below is the OLD linear-slope wording; replace once framing confirmed.}
Finally, we made the two regimes explicit by binning trials into quartiles of the pre-stimulus state (Fig.~\ref{fig:figure4}D,E). The initial deviation---the error the controller directly senses---drove open-loop tracking error steeply across quartiles, but its influence on closed-loop error was sharply reduced (per-session highest-minus-lowest-quartile gap $0.67 \rightarrow 0.21$ within-session standard-deviation units, signed-rank $p = 0.001$; Fig.~\ref{fig:figure4}D). Relative 2--4\,Hz power showed the opposite pattern: closed-loop error climbed across its quartiles just as it did the trial-averaged analysis above, independently of open loop (Fig.~\ref{fig:figure4}E), the one disturbance feedback cannot cancel. The controller therefore behaves as an internal model that estimates and cancels the state-dependent component it can sense---the initial deviation and, as shown above, movement---while the ongoing low-frequency fluctuation sets the residual error that remains.

\begin{figure}[h]
    \centering
    % ===== PLACEHOLDER PANELS (2026-09-07) =====================================
    % Real reconciled panels (13-session cohort) dropped in as placeholders; NOT
    % yet assembled to paperStyle / final A--E lettering. Sources in
    % brain_paper/paper/images/figure4/ via build_figure4.m. Caption below still
    % describes the OLD single-session A--E plan and must be reconciled with the
    % two-mechanism reframe (see PAPER.md "Figure 4 -- build pipeline").
    \fbox{\begin{minipage}{0.98\linewidth}\centering
    \textit{\footnotesize[Placeholder -- provisional panels, 13 controller sessions; lettering \& caption pending reframe]}\\[3pt]
    \includegraphics[width=0.40\linewidth]{images/f4_reject_pooled.pdf}\hfill
    \includegraphics[width=0.56\linewidth]{images/f4_error_decomp.png}\\[3pt]
    \includegraphics[width=0.94\linewidth]{images/f4_error_exemplars.png}
    \end{minipage}}
    % ===========================================================================
    ...
    \small{15 sessions (4 mice); panels requiring motion or spectra use the 11 sessions with motion capture (613 closed-loop trials). Open loop red, closed loop blue; steady-state window 1--3\,s after laser onset. Single-trial insets from a representative session (AL\_0033, 2025-02-26).\\
    ...
    \textbf{B:} Error composition. Unique $R^2$ of initial deviation, motion energy, and relative 2--4\,Hz power for the closed-loop tracking error (RMSE), resolved by window (early 0--1\,s, settled 1--3\,s): initial deviation owns the transient, relative 2--4\,Hz power the settled window, motion neither. Exemplar trials illustrate each factor.\\
    \textbf{C:} Residual decomposition and rejection. The controlled response $A$ splits into the Global disturbance $G$ (predicted from the contralateral hemisphere with no controller input) and the Local component $L = A - G$ (laser and controller effect), shown on one representative trial. Per-trial RMSE to the reference over 1--3\,s is lower in closed loop in all 15 sessions (median ratio $1.39\times$, Wilcoxon signed-rank $p = 6.1\times10^{-5}$).\\
    \textbf{D:} Tracking RMSE (z-scored within session) by pre-stimulus initial-deviation quartile (low~$\rightarrow$~high), open loop and closed loop: open-loop error climbs across quartiles while closed loop is much flatter---feedback removes the dependence on the error it senses (per-session Q4$-$Q1 gap $0.67\rightarrow0.21$, $p = 0.001$). \textbf{E:} The same by relative 2--4\,Hz power quartile: closed-loop error climbs with the state, the disturbance feedback cannot reject.}}
    \label{fig:figure4}
\end{figure}

\todo{ASSEMBLY (2026-09-08, 15-session cohort). 15 sessions / 4 mice (m1--m15, new-rig m14/m15 BLENDED in); motion/spectrum panels use the 11 with motion (613 closed-loop trials). Panels regenerate via \texttt{controller-analysis/build\_figure4.m}: B error-composition = \texttt{cl\_rmse\_factor\_windows.m} (decomp bars + exemplar spectra) + \texttt{cl\_mse\_exemplars.m} + \texttt{cl\_delta\_burst\_explore.m}; C rejection anchor = \texttt{ctrl\_disturbance\_rejection.m} (RMSE-to-ref, 15/15, median $1.39\times$, $p=6.1\times10^{-5}$). Panel A cross-session decoupling now resolved via \texttt{ctrl\_distrej\_quartiles.m} (init-dev quartiles, $p=0.0012$, $n=15$). D/E = quartile classes, init-dev (decouples) + delta (irreducible); see the 4D/E todo above. Current float is a labeled PLACEHOLDER wired to the current panels (\texttt{f4\_reject\_pooled.pdf}=disturbance\_rejection\_pooled\_panel, \texttt{f4\_error\_decomp.png}=cl\_rmse\_factor\_windows, \texttt{f4\_error\_exemplars.png}=cl\_mse\_exemplars); assemble \texttt{Figure4.pdf} at paperStyle once A and D/E lettering settled. Supplementary: locked summary panels (\texttt{f4p1\_error\_decomp}, \texttt{factor\_slope\_*}); contra$\rightarrow$ipsi predictor CV-$R^2$ + distributed co-suppression support treating $G$ as a shared-network disturbance.}
