...
We delivered optogenetic impulse inputs at discrete amplitudes between 0--1.8\,mW powers, randomly interleaved across trials. We found that the mean of the distribution of total inhibition energy per trial, scaled approximately linearly with stimulation amplitude across three sessions (Fig.~\ref{fig:figure2}B; $R^{2} = 0.91, 0.85, 0.93$; slope $= -0.28, -0.50, -0.64\,\%\Delta F/F\,\mathrm{V}^{-1}$). \todo{Add ± CI on each slope estimate (compute from bootstrap or linear-fit 95\% CI).} This linear peak-amplitude relationship supports the design of an open-loop controller calibrated from impulse response data that relies on monotonicity of input output relationship~\citep{astrom1995pid}.
