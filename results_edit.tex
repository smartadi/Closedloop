...
% Moreover, a physiological delay exists between laser application and the resulting effect on calcium imaging output, contributing to the total end-to-end control-loop latency of.
\todo{INCOMPLETE SENTENCE above (commented out): restore and fill in the total end-to-end latency value (80\,ms) and the physiological delay component. Also confirm the 47\,ms figure in the line above is correct — Methods says 80\,ms end-to-end.}
...
% ------------------------------------------------------------
Trial-averaged cortical responses followed a linear, stationary input--output map across both impulse and step stimulus types.
We delivered optogenetic impulse inputs at discrete amplitudes between 0--1.8\,mW powers, randomly interleaved across trials. We found that the mean of the distribution of total inhibition energy per trial, scaled approximately linearly with stimulation amplitude across three sessions (Fig.~\ref{fig:figure2}B; $R^{2} = 0.91, 0.85, 0.93$; slope $= -0.28, -0.50, -0.64\,\%\Delta F/F\,\mathrm{V}^{-1}$). \todo{Add ± CI on each slope estimate (compute from bootstrap or linear-fit 95\% CI). Also: (1) justify n=3 sessions — either add remaining sessions or state explicit inclusion criterion; (2) Fig.~2B caption says ``Peak \%\,$\Delta F/F$ suppression'' but Methods now defines this metric as \emph{inhibition energy} (0--200\,ms mean) — harmonize terminology everywhere.} This linear peak-amplitude relationship supports the design of an open-loop controller calibrated from impulse response data that relies on monotonicity of input output relationship~\citep{astrom1995pid}.
...
\begin{figure}
    ...
    \textbf{B:} \todo{CAPTION: ``Peak \%\,$\Delta F/F$ suppression'' should be ``Inhibition energy (mean $\Delta F/F$, 0--200\,ms post-onset)'' to match Methods definition. Also update ``mean $\pm$ SD'' to ``median with 2.5th--97.5th percentile bounds'' if that is what is plotted.} Peak $\%\,\Delta F/F$ suppression versus stimulation amplitude for three sessions. Points show mean $\pm$ SD across trials ($\sim$50 trials per amplitude); lines show the fitted linear input-output relationship (see Methods). $R^{2} = 0.91, 0.85, 0.93$ across sessions.\\
...
Step inputs applied over 3-second trial windows produced trial-averaged trajectories that remained within a bounded set around the session mean (Fig.~\ref{fig:figure2}C), consistent with marginally stable oscillatory dynamics in biologically bounded cortical circuits. \todo{Add note that the 3\,s window is too short to observe steady-state settling — the step response does not reach a clear plateau within the trial, so steady-state gain cannot be directly read off. Acknowledge this limitation explicitly.}
\todo{FACTUALLY WRONG CLAIM — rewrite this sentence. Analysis (FINDINGS.md, variance\_mse.m) shows: pre-onset slope $= -0.04 \pm 0.14$\,($\Delta F/F$)$^2$/s $\approx$ zero; post-onset slope $= -0.57 \pm 0.24$\,($\Delta F/F$)$^2$/s, consistently negative across $n=13$ sessions. OL stimulation itself \emph{reduces} variance during the stim window. The correct claim is: ``Open-loop stimulation modestly reduced across-trial variance during stimulation (post-onset slope $-0.57 \pm 0.24$\,($\Delta F/F$)$^2$/s, vs.\ near-zero pre-onset slope $-0.04 \pm 0.14$, $n=13$ sessions); closed-loop feedback produced a further and more consistent reduction.''} Under open-loop step stimulation, trial-to-trial variance did not change around stimulus onset (Fig.~\ref{fig:figure2}C), indicating that optogenetic input did not destabilize the system.
...
\begin{figure}
    ...
    \caption{Closed-loop versus open-loop cortical activity control \todo{INCONSISTENCY: ``3 mice'' here but Results text says ``two mice''. Resolve before submission.} (3 mice, 13 sessions).\\
    \small{
    \textbf{A:} Single-trial example of open-loop (red) and closed-loop (green) $\Delta F/F$ responses. Gray shading marks the 3\,s stimulation window; dashed line indicates the target $-5\%\,\Delta F/F$. Bottom traces show the corresponding optogenetic input signals.\\
    \textbf{B:} Trial-averaged $\Delta F/F$ responses under open-loop (red) and closed-loop (green) control for a representative session. Bold lines show the trial mean; thin lines show individual trials.\\
    \textbf{C:} Trial-averaged optogenetic input signals for open-loop (pink) and closed-loop (blue) conditions. Bold lines show the mean input; thin lines show individual trials.\\
    \textbf{D:} Cross-trial variance of $\Delta F/F$ as a function of time for open-loop and closed-loop conditions, for a representative session. Gray shading marks the stimulation window.\\
    \textbf{E:} \todo{MSE WINDOW: caption says ``over the 3\,s stimulation window'' but locked-in decision (MEETINGS 2026-05-08) is $t = +1\,\text{s}$ to $+3\,\text{s}$ post-laser onset. Update caption and verify figures were regenerated with the correct window.} Distribution of per-trial tracking MSE (mean squared error over the 3\,s stimulation window) for open-loop and closed-loop conditions, shown as violin plots. Points indicate individual trials.\\
    \textbf{F:} Cross-trial variance during the stimulation window, averaged across all 13 sessions, for open-loop and closed-loop conditions.\\
    \textbf{G:} Per-trial MSE distributions pooled across all 13 sessions, shown as violin plots.\\
    \textbf{H:} Mean absolute tracking error per unit time as a function of time within the trial, averaged across all sessions (bold) with individual session traces (thin lines).\\
...
These improvements held across all recorded sessions (Fig.~\ref{fig:figure3}G) and required only modest gains ($K_{p} = \mathbf{[X]}$\todo{Fill in Kp — extract from .mat session data}, $K_{i} = \mathbf{[X]}$\todo{Fill in Ki — extract from .mat session data}), suggesting that the approximately linear, stationary input--output structure identified empirically was sufficient to support stable PI feedback.
