...
We designed a proportional--integral (PI) feedback controller with open-loop compensation to drive cortical activity toward the reference $-5\%\,\Delta F/F$ (Fig.~\ref{fig:figure1}E).
...
We delivered optogenetic impulse inputs at five amplitude levels (0--2.7\,V), randomly interleaved across trials, and found that the trial-averaged peak $\%\,\Delta F/F$ suppression scaled approximately linearly with stimulation amplitude across three sessions (Fig.~\ref{fig:figure2}B; $R^{2} = 0.91, 0.85, 0.93$; slope $= -0.28, -0.50, -0.64\,\%\Delta F/F\,\mathrm{V}^{-1}$). \todo{Add ± CI on each slope estimate (compute from bootstrap or linear-fit 95\% CI). Justify n=3 sessions or include remaining sessions with selection criterion.} This linear peak-amplitude relationship supports a feedforward controller calibrated from impulse response data.
...
Under open-loop step stimulation, trial-to-trial variance did not change around stimulus onset (Fig.~\ref{fig:figure2}C), indicating that optogenetic input did not destabilize the system. \todo{Quantify pre- vs.\ post-onset variance ratio or p-value; state explicitly that this claim applies to open-loop only.}
...
% ------------------------------------------------------------
\subsection*{Pre-stimulus brain state predicts trial outcome}
% ------------------------------------------------------------
