...
[0,\,0.3] \times [0,\,0.1]$ (Fig.~\ref{fig:cost_landscape}A), then
refined using the model-free zeroth-order search described in
Algorithm~\ref{alg:zo}.
...
\begin{figure}[!ht]
    ...
    \begin{subfigure}[t]{\linewidth}
        ...
        \caption*{\textbf{B}}
    \end{subfigure}
    \caption{\textbf{Controller gain optimization.}
        \textbf{A:} Empirical cost landscape $\hat{J}(K)$ over the gain
        grid $K \in [0,\,0.3]\times[0,\,0.1]$, used to initialise the
        zeroth-order search for a representative session.
        \textbf{B:} Trajectory of candidate gain vectors during the
        zeroth-order search; each point shows $\hat{J}(K)$ for one
        candidate, illustrating convergence toward a low-cost gain.}
...
best-performing gain from the grid sweep (Fig.~\ref{fig:cost_landscape}A).
We considered a controller acceptable if $\hat{J}(K) \leq \hat{J}(0)$,
