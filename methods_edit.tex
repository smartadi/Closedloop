...
cost is reported normalized by the number of samples in the evaluation window
as a per-trial mean-squared error (MSE). The per-trial tracking cost for an
...
cross-session MSE distributions (\texttt{er\_ncDfk}/\texttt{er\_wcDfk}), and a
...
the cost-surface minimum is identical whether scored as RMS deviation or
mean-squared error.
...
cross-session figures we pooled per-trial MSE arrays, cross-trial
...
tracking MSE (windowed, $t = +1$--$+3\,\text{s}$) on this pre-stimulus
...
related per-trial tracking MSE to concurrent motion energy. For each session
with face-video motion capture, we averaged the z-scored motion energy over the
stimulation window of each trial and z-scored the per-trial MSE within session
...
differences). We regressed within-session normalized MSE on motion energy
...
quartile, and compared open- versus closed-loop MSE within the highest motion
...
MSE ratios per window) used the two-sided Wilcoxon signed-rank test;
unpaired comparisons of pooled trial distributions (e.g.\ open- vs.\
closed-loop MSE within a motion quartile) used the Wilcoxon rank-sum test.
Trend and dependence relationships (per-trial mean vs.\ acquisition order;
MSE vs.\ pre-stimulus variance; MSE vs.\ motion energy) were quantified by
