...
For input-output mapping we designed experiments where impulse reponse data was collected for three session in two different mice.
We defined inhibition energy as $\sum_{t=0}^{T} y_t^2$, where $T = 200ms$ was the determined as the settling time of the trial-averaged response for any input amplitude.
...
\todo{MISSING: port the zeroth-order gain-search algorithm pseudocode from \texttt{methods.tex} into this file as an \texttt{algorithm} block with label \texttt{alg:zo}. Also add the gain-grid figure reference (\texttt{fig ref: gain grid figure}) in the two \texttt{todo}s above once the figure is finalized.}
